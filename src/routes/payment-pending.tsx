...
import { z } from "zod";
import { getFundingStatus } from "@/lib/wallet.functions";
...
function PaymentPending() {
  const { reference: searchRef } = Route.useSearch();
  const navigate = useNavigate();
  const fetchStatus = useServerFn(getFundingStatus);
  const [reference, setReference] = useState<string | undefined>(searchRef);

  useEffect(() => {
    if (searchRef) {
      localStorage.setItem("pending_funding_ref", searchRef);
      setReference(searchRef);
    } else {
...
function Success({ amount, onContinue }: { amount: number; onContinue: () => void }) {
  return (
    <>
      <div className="w-20 h-20 rounded-full bg-emerald-50 flex items-center justify-center">
        <CheckCircle2 className="text-emerald-600" size={40} />
      </div>
      <h1 className="mt-6 text-xl font-bold">Wallet funded!</h1>
      <p className="mt-2 text-sm text-muted-foreground">
        {formatNaira(amount)} has been credited to your wallet.
      </p>
      <button
        onClick={onContinue}
        className="mt-8 w-full max-w-xs gradient-primary text-primary-foreground rounded-full py-3.5 text-sm font-semibold shadow-glow flex items-center justify-center gap-2"
      >
