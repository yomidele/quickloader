...
const MIN_FUND = 500; // Minimum funding amount in Naira
const QUICK_AMOUNTS = [500, 1000, 5000, 10000]; // Quick amount buttons
...
async function startFunding(token: string, amount: number) {
  ...
  if (!response.ok) {
    ...
    throw new Error(error.error || 'Failed to initiate funding');
  }
